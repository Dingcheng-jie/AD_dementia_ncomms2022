...
class Multask_Wrapper:
    ...
    def load_MLP_weights(self):
        for i, task in enumerate(self.tasks):
            ...
            print('loading ', target_file)
            try:
                self.MLPs[i].load_state_dict(torch.load(target_file, map_location=lambda storage, loc: storage.cuda(self.device)))
            except:
                self.MLPs[i].load_state_dict(remove_module(torch.load(target_file, map_location=lambda storage, loc: storage.cuda(self.device))))
